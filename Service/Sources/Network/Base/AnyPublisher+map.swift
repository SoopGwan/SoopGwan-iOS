...
extension AnyPublisher where Output == Moya.Response, Failure == SoopGwanError {
    func map<D: Decodable>(
        _ type: D.Type,
        atKeyPath keyPath: String? = nil,
        using decoder: JSONDecoder = JSONDecoder(),
        failsOnEmptyData: Bool = true
    ) -> AnyPublisher<D, SoopGwanError> {
...
extension AnyPublisher where Failure == SoopGwanError {
    func unwrapThrowable<T>(throwable: @escaping (Output) throws -> T) -> AnyPublisher<T, SoopGwanError> {
